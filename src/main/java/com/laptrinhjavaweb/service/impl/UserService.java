...
//        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1, "USER");
    ...
    @Override
    public List<StaffResponseDTO> getBuildingAssignedStaff(Long buildingId) {
        List<StaffResponseDTO> result = new ArrayList<>();
        // List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1, "USER");
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1, "STAFF");
        staffs.stream().forEach(staff -> {
            StaffResponseDTO staffResponseDTO = new StaffResponseDTO();
            staff.getBuildings().stream().forEach(building -> {
    ...
    @Override
    public List<StaffResponseDTO> getCustomerAssignedStaff(Long customerId) {
        List<StaffResponseDTO> result = new ArrayList<>();
        List<UserEntity> staffs = userRepository.findByStatusAndRoles_Code(1, "STAFF");
        staffs.stream().forEach(staff -> {
            StaffResponseDTO staffResponseDTO = new StaffResponseDTO();
            staff.getCustomers().stream().forEach(customer -> {
